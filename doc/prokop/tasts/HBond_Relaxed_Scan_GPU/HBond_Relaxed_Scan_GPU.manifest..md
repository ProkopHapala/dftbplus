...
- [x] `md_step` velocity-Verlet — **FIXED 2026-09-14:** true VV (drift with stored a(t), eval, ½(a+a′) kick); `md_f_prev` state disarmed on any set_coords. Was full-force kick (not VV).
...
- [*] DONE 2026-09-13: **W1. Kill the per-iteration blocking Jacobi diagnostic read.** `eigh_solve→report_jacobi` calls `read_buffer` → `queue.finish()` every N>64 SCC iter — forcibly drains the async pipeline (P0). Keep diag on device; host reads it only at solve end or when a nonzero stop was flagged. Policy for stops: `stop=4` nonfinite → hard-fail replica; `stop=2` max-sweeps → fail unless residual within accepted f32 floor; `stop=1` warm → explicit GPU cold-solve recovery, not just a warning. Stop zeroing off-diagonals on bad exits (destroys diagnostics).
- [*] DONE 2026-09-13: **W2. GEMM: remove Kahan → 4 independent f32 FMA accumulators.** Then fix the uncoalesced transposed loader (`trans_a=1` does stride-N loads; `BᵀH` runs every warm iter): remap work-item→tile so the fast index varies along memory, store transposed into `As` — or maintain `Bᵀ` and use the NN kernel. Benchmark both.
- [*] DONE 2026-09-13: **W3. Direct-Jacobi precision/WG A/B on the PRODUCTION kernel** (not deprecated tiled): `JACOBI_PREC` 0 vs 1 — inputs are already f32 so f64 c/s only prevents formula rounding; if marginal, prefer f32 early sweeps + one f64 refinement sweep. WG ∈ {128,256,512(,1024)} at N≈87 — batch=19×WG=256 uses only 19/82 SMs; query kernel-specific WG limit/preferred multiple/local/register usage. Metrics: event time, HC−SCε residual, CᵀSC−I, relative PES energy, force parity, SCC iters.
- [*] DONE 2026-09-13: **W4. Chunked host convergence control (per USER amendment).** Store per-replica `{active,status,rms,rms_hist,niter}` on device; enqueue ~8–10 SCC iters without host read; then one compact status read. Requires **active-gated SCC GEMM** (`batched_gemm_active`; keep ungated GEMM for Löwdin/metric work) — today done replicas still burn GEMM tiles. Do NOT add a dedicated GPU convergence kernel (USER decision) — residual rides on the Mulliken/DIIS pass.
- [~] PARTIAL 2026-09-13: **W5. Fermi μ+f_k fused into the Jacobi tail** (I7); skip the integer-occupation bitonic sort on the smeared path. **As built:** separate `fermi_occ_batched` kernel (f64 bisection ×40 on eig_diag, writes occ_w+mu_out) — kills the host roundtrip (warm scc 6.16→1.36 ms profiled at batch=19) but still needs `extract_diag` first + one extra launch; Jacobi-tail fusion remains (see §15 R5).
- [~] **W6. FIRE control device-resident** (I8) — **control part DONE 2026-09-14** (see §15.B R8a entry): dt/α/n_pos/mode adapted in the `fire_reduce` tail, `park` mask from `scc_ok`, host keeps only the stat read for convergence/finiteness. **Remaining:** geometry-active mask — parked/converged replicas must not rebuild γ, H0/S, pair geometry, metric repair while others relax.
- [~] **W7. Metric repair cheaper + certified on device** (I13) — **bound-certify DONE 2026-09-14**: `metric_residual_batched` now computes true row-sum ‖M−I‖∞ (was elementwise max — that does NOT bound eigen-residuals). Both Newton repairs early-exit via `e₁≤¾e₀²+¼e₀³`: when the bound < tol for ALL replicas the step applies unconditionally (device copy) — T2/G1/e1 GEMMs + second readback skipped. One blocking read remains per attempt (host must branch on the bound); full device-side certification would need the acceptance decision itself on GPU (speculative re-solve — not yet done).
- [x] **W8. Geometry refresh fused into H/S assembly — DONE 2026-09-14.** `assemble_pairs_geom` (shared `assemble_pair_body` with the legacy host-geometry `assemble_pairs`) computes r,l,m,n from `buf_coords_bohr` in-kernel; `refresh_pair_geom` no longer runs in `GpuDftb::assemble` (kernel kept for GpuForceDriver). `onsite_diagonal` enqueued once (`onsite_done`) — it is geometry-independent. `fire.assemble` 0.14→0.10 ms/call. PairEntry keeps its r/l/m/n fields for the test/legacy consumers.
- [~] **W9. Atomic-free fused forces** (I9/I10): **W9a DONE 2026-09-14** — `force_pairs_fused`: one SK interp + one dS/dR serves `D·dH + (½(V_i+V_j)D − W)·dS`; one atomic add per endpoint. `fire.force_kernels` 0.51→0.37 ms/call (−27%), E(r) unchanged ~1e-6 Ha; the two component kernels remain for `GpuForceDriver` component tests. **Remaining:** atomic-scatter → per-atom gather, `e_atom` emission, p–p register spill, `PAIR_WG` tuning.
- [x] **W10. Allocation sweep — DONE 2026-09-14** (§15.B): `alloc_count` runtime counter + `test_gpu_dftb_no_loop_allocs` (delta=0 across scc/eval/fire). GPU-alloc audit clean; host `vec![1i32]` per check_jacobi → persistent `ones_batch`. Remaining (minor): `reset_diis` device fill, persistent `hist/stagnant/done` in `scc_mix_inner` (host Vec allocs only).
- [x] **W11. Fallback-trap removal — DONE 2026-09-14** (§15.B): `scc()` honest one-shot; explicit `scc_retry_failed` / `scc_with_retry` used by `relax` and `gpu_scc`. I14 legacy-API routing remains open.
- [~] **W12. GPU energy/residual reductions where cheap** — **energy part DONE 2026-09-14**: `energy_reduce_batched` (WG/replica, f64 accum) writes `e_scal[batch×4]` = e_band/mts/Δq·V/q0·V — one readback replaces the six per-eval tails (eig_diag+eig_rho+occ_mask+occ_w+dq·v+q0·v). `relax.eval` host 0.479→0.431 ms/call, relax readbacks 2788→2068, energies unchanged. **Remaining:** `{q_rms,q_max}` still does two atom-vector downloads (eval-only, ~50µs); SCC-residual scalars already live in `rms`/`diis_flag`.
...
- [x] GEMM Kahan vs 4×FMA — **decided 2026-09-13:** Kahan measured no-cut on δ_CH (report addendum); 4-acc FMA is production. [closed]
- [x] direct Jacobi prec0-vs-1 — **decided R8a:** PREC=0 default, −28% Jacobi, ΔE≤2e-6 Ha. WG: 512 measured 1.55× vs 256 (W3) — already in use. [closed]
- [x] SCC host-control vs chunked — **decided:** chunked (mix=0) is production; mix=1/2 kept as reference. [closed]
- [x] basis repair 5-GEMM-verified vs 3-GEMM-bounded — **superseded by W7 2026-09-14:** 3-GEMM + Newton-bound certify (row-sum ‖·‖∞, `e₁≤¾e₀²+¼e₀³`); measured-verify only when the bound fails. [closed]
- [x] pair-force atomic vs pair-write+atom-gather vs WG-per-atom — **decided W9b 2026-09-14:** per-pair write + CSR atom gather won (−42% force stage). [closed]
- [x] stages to instrument — **DONE 2026-09-14:** evt profiler covers all of these; stage table in labbook 2026-09-14 entries. [closed]
- [ ] **NEW A/B backlog (post-2026-09-14):** Fermi-tail cost (≤24 Newton × f64 WG-reduce chains — the remaining Jacobi fixed cost at warm nsw=0; candidates: n-lane reduce not 512, warp-shuffle reduce, adaptive iters) · `PAIR_WG` tuning + p–p register spill (W9b residual). **DONE 2026-09-14:** Jacobi prologue fused (1 pass + 1 packed reduce; dead off-diag zero pass removed) → warm scc −27% @256; `e_atom` emitted+validated (|ΔTr(DH0)|=2.4e-7).

## 15. GPT-5.6 review #3 — profiler-driven work order (2026-09-13, chat 4008–4700)

**Context.** The built-in stage profiler (`RUST_DFTB_PROF=1|mark`) gave the first honest breakdown at batch=19, N=86. Reviewer's reading, endorsed by the user: **for N≈86, Jacobi should be the dominant irreducible cost; anything else comparable to it is a design bug.** Measured: `hscc` ≈ 0.7 ms/iter, `occ` ≈ 0.68 ms/iter (pre-W5), `jacobi` ≈ 0.48 ms/iter — both non-eigensolver stages were LARGER than the eigensolver. That is the red flag, now partly fixed.

### 15.A What the profile actually said

- `occ_host` (host Fermi bisection) was 131k f64 `exp()`/iter on the CPU — 80 bisections to find 19 scalar μs; absurd. → **W5 fixed the host roundtrip** (device kernel). Residual: still a separate kernel + extract_diag + select_occ; reviewer wants Jacobi-tail fusion + ~8–12 safeguarded Newton steps (warm-start μ from previous iter), f32 `exp` sufficient (f_k is f32 anyway); skip `exp` for |x|>20 (deep occ/virt).
- `hscc` (fused Δq→γΔq→H_scc) ≈ 0.7 ms/iter for ~1.7 MB of traffic ≈ **2.4 GB/s effective bandwidth on a 3090** — suspicious. **BUT (user correction): the "19 WGs on 82 SMs" occupancy argument is an artifact of the batch=19 TEST, not necessarily of the architecture.** Production is hundreds of scan geometries in parallel — at batch ~300–500 the one-WG-per-replica layout already gives hundreds of WGs and saturates; multi-WG-per-replica would then be *worse* (redundant V compute + more complexity for the same occupancy). **Measure at production batch (≥200) before concluding the layout is wrong.** What stays true at ANY batch: only ~29/256 lanes work during the dq/γΔq phases (small dead fraction), and per-replica WG choice should be revisited with the *measured* production batch, not the test's.
- mark-vs-tick mode also showed per-stage host enqueue+set_arg ≈ 0.15–0.2 ms — **repeated `set_arg` of invariant buffer handles every iteration is pure overhead** (buffer handles don't change between iters, only contents do).

### 15.B Work order for tomorrow (priority order)

- [x] **R1. Prebind invariant kernel args — no `set_arg` for persistent buffers inside the SCC loop.** **DONE 2026-09-13(c)** — all io buffers bound at `GpuSccPlan::new`; per-solve scalars via `bind_solve_params`/`bind_mix_params`; warm/cold GEMM + Jacobi run on separate persistent handles. Measured (mark mode, batch=19): `scc.hscc` host stage 0.68→0.040 ms/call, all per-iter stages ≤40 µs host — the loop is now pure enqueues and `chunkend` carries the real GPU work (~0.9 ms/iter). Reviving the stale test paths also fixed two real bugs: `scc_step`/`scc_step_diis` never committed q_next, and the test-only `GpuForceDriver` gamma-deriv kernel was missing its 4 D8 spline args.
- [x] **R2. `dq_v_hscc` — CLOSED BY MEASUREMENT (2026-09-13d/e, evt mode).** True device time: `scc.hscc` = 0.014 ms/call = **1.5% of the iteration** at batch=19 — the tick-mode 0.68 ms was a set_arg+sync artifact, not kernel work. Even before scaling to production batch it is irrelevant; at batch 300–500 the one-WG-per-replica layout saturates anyway. No multi-WG variant, no fusion — leave it.
- [x] **R3. OpenCL event profiling — DONE 2026-09-13(d).** `RUST_DFTB_PROF=evt`: queue created with CL_QUEUE_PROFILING_ENABLE, marker events per `prof_tick`, timestamps drained lazily at natural syncs (`finish`/`read_buffer`/report — never inline waits). `[prof-dev]` table prints alongside the host `[prof]` table. Immediately corrected the attribution error above: Jacobi 0.636 ms/call (67% of iter) is the real elephant; `hscc` 1.5%.
- [-] **R4. Fuse H_scc into the first projection GEMM — DEPRIORITIZED.** Same measurement: hscc 1.5% + projection GEMMs 3.5% of iter — fusing saves <0.05 ms/iter of 0.84. Not worth the complexity; revisit only if the ratio changes at production batch.
- [x] **R5. Jacobi-tail Fermi — DONE 2026-09-13(e).** `jacobi_cyclic_global_batched` tail: staged spectrum → safeguarded Newton (2×f64 WG reductions/iter, bisection fallback, μ warm-started from persistent `mu`) → occ_w+mu written on exit. +1 µs/call on Jacobi; deletes fermi_occ (0.067 ms) + select_occ + in-loop extract_diag. `RUST_DFTB_FERMI_REF=1` = W5 certified-reference path. `occ_normalize_batched` gained use_w predicate; `measure` derives its mask from occ_w>0.5. n≤64 local Jacobi has no tail → W5 kernel still used there. E(r) identical within 1e-6 Ha.
- [x] **R6. Rayleigh out of the SCC loop — DONE 2026-09-13(e).** Verified: eig_rho is read only by `energy_from_state`/`build_edm`, both on the finalized state (which already runs occ_rayleigh). In-loop launch deleted — every committed step leaves the state stale anyway (state_fresh=false → eval re-finalizes). −0.040 ms/iter.
- [x] **R7. Commit fused into DIIS — DONE 2026-09-13(e).** `diis_step_batched` arg6 bound to `q_gpu`: mixed iterate written in place for still-active replicas; converged/failed replicas return before the mix (same gate the commit used). `commit_q_batched` deleted from the chunk loop + `scc_step_diis`; mix=0 reference branch skips set_active+commit (stale q_next would corrupt q_gpu). mix=1/2 keep the q_next/commit path. −1 launch, −0.004 ms/iter.
- [x] **R8a. `JACOBI_PREC=0` A/B — DONE 2026-09-14, default flipped to 0.** GC PT scan batch=19 evt: Jacobi 0.655→0.450 ms/call (**−31 %**), wall 74.5→57–67 ms/pt (run noise ±20 %); max |ΔE| ≈ 2e-6 Ha across all 19 points — the renorm + Rayleigh repair absorbs the coarser in-loop residual, exactly as predicted. `RUST_DFTB_JACOBI_PREC=1` / `gpu_jacobi_prec(1)` remain the certified references. Fix on the way: legacy `gpu_solve_scc_*` callers were stale (`extract_diagonal`/`mulliken` missing `active`, `direct_jacobi` 7-arg) — repaired.
- [x] **W6. Device-resident FIRE control — DONE 2026-09-14.** `fire_reduce_batched` tail now adapts {dt, α, n_pos, mode} per replica in `fire_ctl` (constants N_MIN/F_INC/F_DEC/α_start/DT_MAX on device); parking gates on a new `park` buffer mirroring `scc_ok` (synced wherever scc_ok changes). `fire_step` is now: forces → reduce → ONE stat read (convergence + finiteness check) → apply — the mid-step ctl write_buffer is gone. `fire.apply` host 0.098→0.010 ms/call. **Bug caught on the way:** storing the mode-2 masked alpha (0) back into ctl zeroed the adaptation state — alpha now stores TRUE α, apply masks it for mode 2. Scan energies match prec1 to ~1e-6 Ha.
- [x] **W10. Allocation sweep — DONE 2026-09-14.** `GpuRuntime::alloc_count` (AtomicU64) counts every device buffer alloc; `test_gpu_dftb_no_loop_allocs` asserts delta=0 across scc→eval→fire→scc→eval. Host-side `vec![1i32;batch]` per check_jacobi call → persistent `ones_batch`. GPU alloc audit clean.
- [x] **W11. Explicit retry — DONE 2026-09-14.** `scc()` is now honest one-shot; the failed→nearest-converged reseed moved to explicit `scc_retry_failed(&prev,…)`; production call sites (`relax`, `gpu_scc`) use `scc_with_retry` so the fallback is visible at the call site. `gpu_bench`/vibrations stay honest.
- [x] **Profiler v2 — DONE 2026-09-14.** `evt` mode now prints ONE merged table: host ms + dev ms + n + per-call both sides + dev% per stage (fixes the `gpu_total` seconds-as-ms bug). `RUST_DFTB_PROF_OUT=<file>` appends the report for A/B diffing.
- [x] **W9a. Fused pair force — DONE 2026-09-14.** `force_pairs_fused` computes `D·dH + (½(V_i+V_j)·D − W)·dS` in one pass — one SK-table load, one derivative block, one atomic add per endpoint (was two kernels doing the same interpolation twice + double atomics). `fire.force_kernels` 0.508→0.372 ms/call. The `force_pairs`/`force_pairs_scc_shift` kernels stay for component-level tests.
- [x] **R8b. Jacobi internals — measured 2026-09-14** (env: `RUST_DFTB_JACOBI_REPORT=1` per-eval histogram; `RUST_DFTB_JACOBI_OFF_TOL` env now overrides the exit threshold). Findings at **production batch 256** (GC, N=86): **warm-started H-Jacobi exits at 0 sweeps** — entry off/‖A‖_F ≈ 3.5e-7, already below every tol in {1e-4…1e-7}; cold solves take 1 sweep; stop=0 on all replicas; `JACOBI_OFF_TOL` is a no-op on the warm path (timing noise ±30% dominates). **The 58% Jacobi dev share is FIXED kernel overhead — V=I init (n²), ‖A‖_F+off reductions (2×n²), diag write — not rotations.** Scaling: 40µs→1.2µs/iter/system from batch 1→256. Follow-up target: skip/early-out the V-init+off-norm prologue when a cheap pre-check shows the warm basis already diagonal (or fold V-write into eigh_finish's matmul when nsw=0).
- [x] **W8.** DONE 2026-09-14 (above): `assemble_pairs_geom` + `force_pairs_fused` compute r,l,m,n from coords; refresh launch dead; `onsite_done` once-only.
- [x] **W6b.** DONE 2026-09-14: `park` mask (scc_ok mirror) now gates `assemble_pairs_geom`, `force_pairs_fused`, `build_gamma_batched`, `force_gamma_deriv_batched`, `force_repulsive_batched` — parked replicas skip γ/H0/S/force work (stale = frozen-geometry-correct). `set_coords` un-parks (host geometry invalidates failure). Remaining: `set_geometry`'s warm-B repair GEMMs/readbacks still run per-replica-invariant — parked replicas contribute ~0 error to the batch reduction, so the work is small; real fix is W7.
- [x] **W9b.** atomic-scatter → per-atom gather — DONE 2026-09-14: `force_pairs_fused` writes per-pair forces to flat `pair_f` (one flat buffer, per-bucket `pair_base`); new `force_gather_pairs` reduces via a host-built atom→pair CSR in deterministic order, sole writer of the pair term (`gamma_deriv`/`repulsive` still atomic-add their 1–2 terms/atom). `fire.force_kernels` 0.51→0.30 ms/call (−42% vs pre-fusion). `e_atom` DONE same-day (pair_e + onsite in gather; validated vs Tr(D·H0), |Δ|=2.4e-7). Remaining: p–p register spill, `PAIR_WG` tuning.
- [x] **W13.** DONE 2026-09-14: `diis_step_batched` solve is now pivoted f64 MGS-QR on the residual-history matrix (working columns in new `diis_work` f64 buffer) — dependent columns drop individually by pivot magnitude; Σc=1 solve = two κ(R)-triangular solves, never forms RᵀR. Drop-oldest retry machinery removed. All 45 GPU tests pass; scan energies unchanged.
- [x] **W7.** Bound-certified metric repair — DONE 2026-09-14 (§15.9 above): row-sum ‖·‖∞ + ¾e²+¼e³ early-accept, one read/attempt. Residual: host still reads e0 once per attempt to branch.
- [x] **W12.** DONE 2026-09-14: `energy_reduce_batched` f64 scalar tails (e_band, mts, Δq·V, q0·V) → `double e_scal[batch*4]`, one readback. (Not piggybacked on the W9b gather — energy runs per-eval, forces only in FIRE; a per-eval WG/replica kernel is the right home.)
- [x] **W13.** DONE 2026-09-14 (above): pivoted f64 MGS-QR DIIS.
- [x] **W6b.** DONE 2026-09-14 (above): `park` gates pair assembly/forces/γ/repulsive; `set_coords` un-parks.

### 15.C Acceptance budget (target shape per SCC iteration)

- Jacobi ≈ 60–80 % of iteration time; projection GEMMs ≈ 10–25 %; everything else < 20 %.
- Fermi incremental cost < 5 % of Jacobi. `hscc` ≤ 0.1 ms at the production batch — else fused away by R4.
- ≤ 5–6 kernel launches per iteration (end-state: ~4: fused-projection, T·B, Jacobi+Fermi, charge+DIIS+commit, [S-norm when needed]).
- Zero set_arg for invariant args, zero host contact inside a chunk, zero allocs — I2/I3 test enforcement.
- **Benchmark all per-iter claims at production batch (~200–500), not the batch=19 test** — occupancy conclusions drawn at batch 19 were misleading (R2).

### 15.D Known semantic deltas of the chunked loop (mix=0 only)

- **mix=0 is the production path; mix∈{1,2} are unchunked reference paths** (host f64 DIIS / simple mixer still do per-iteration host reads — keep them as certified references, do not "optimize" them away).
