...
- [ ] **R1. Prebind invariant kernel args — no `set_arg` for persistent buffers inside the SCC loop.** `enq_dq_v_hscc` alone does 6 `clSetKernelArg`/iter of unchanged handles; same pattern in density/mulliken/rayleigh/diis. Bind at plan construction; per-call only scalars and genuinely-swapped buffers. Where a kernel is invoked with different static arg sets (warm vs cold projection, integer vs Fermi density), build **separate Kernel handles** — handle duplication is cheap, API traffic per iter is not.
- [ ] **R2. `dq_v_hscc` — measure first at PRODUCTION batch, then decide.** The batch=19 "19 WGs / 82 SMs" underfill does not generalize: at scan batch ~300–500 the single-WG-per-replica layout already saturates and is simpler. **Do:** (a) benchmark `hscc` at batch ∈ {19, 64, 200, 500} first — if it shrinks to ≤10–20% of iter at production batch, the layout is FINE, don't touch it; (b) only if it stays dominant at production batch, consider G=2–4 WGs/replica (each redundantly computes the 29² dq/V in __local, owns a slice of N²) — benchmark G ∈ {1,2,4} × WG ∈ {64,128,256} at the target batch. The deeper alternative remains R4 (never materialize H_scc).
- [ ] **R3. OpenCL event profiling for TRUE per-kernel GPU times** — create the queue with profiling enabled (or `kernel.cmd().enew(&ev)`), attach events at enqueues, read timestamps AFTER the normal chunk sync (never wait on an event inline). Splits stage time into host-enqueue vs GPU-execution; the clFinish marks stay as the coarse instrument. This is the "proper profiler" that ncu would have been if counters weren't admin-locked.
- [ ] **R4. Fuse H_scc into the first projection GEMM.** `H_scc = H0 + ½S(V_i+V_j)` can be synthesized while the `BᵀH` kernel loads its H tile — each projection WG redundantly computes γΔq in __local (29² FMAs). Removes the whole N² materialization + the hscc launch + global round-trip. This is the end-state replacement for R2.
- [ ] **R5. Finish W5 → Jacobi-tail Fermi.** Move μ-solve + f_k write into the tail of `jacobi_cyclic_global_batched` (the WG already owns the full spectrum in local); delete `extract_diag` + `select_occ` on the smeared production path (integer mask unneeded — D/W weight all columns). Safeguarded Newton + bracket fallback, μ warm-started from previous iter (persistent `mu` buffer), ~8–12 iters max. Keep CPU f64 Fermi + the W5 kernel as the certified reference behind a flag.
- [ ] **R6. Rayleigh (occ_rayleigh) out of the SCC loop.** ρ_k is consumed only by final band-energy/W — compute it once on the certified final state inside finalize, not every iteration (−0.06 ms/iter, −1 launch). Verify nothing mid-iteration reads eig_rho (occ_repair path check!) before moving.
- [ ] **R7. Fuse commit into DIIS.** `diis_step_batched` already computes rms and marks `active`; write the mixed q directly to `q_gpu` for non-converged replicas and leave `q_gpu=q_n` for converged ones — deletes `q_next` buffer, `commit_q_batched` launch, and one global round-trip per iter.
- [ ] **R8. Then W6 (device FIRE) → W9 (gather forces + e_atom).** Only after the loop is ~5 launches/iter revisit Jacobi occupancy — and note the same batch-size caveat applies: at production batch the one-WG-per-replica Jacobi already fills the machine; multi-WG-per-replica is only worth it at small batch.
