...
**Honest assessment (2026-09-18): the Phase-B end-to-end gain is MARGINAL (~3%) and not worth pursuing further on this workload.** The masked-launch path already early-exited dead workgroups cheaply, so removing them saves less than the kernel-level sweep suggested; the dominant per-iteration cost is the real kernels on active slots, not scheduling. The machinery is kept (correctness-verified, near-free with the monotonic guard, and a prerequisite for the Phase C slot-pool scheduler where compaction is structural, not an optimization), but **the remaining SCC-loop cost is dominated by the real kernels + host-side orchestration — the next lever is harness overhead (syncs, transfers, per-iteration host work), not launch geometry.**
...
- [x] **JACOBI_PREC=0 — rejected at equal accuracy**: not faster AND 10–30× worse eig parity (N246 cold: 7.8e-3 vs 2.6e-4). prec=1 stays. *(2026-09-18 note: default was later changed to PREC=0 anyway — policy decision, f32-is-the-architecture; prec=1 remains the accuracy-reference knob. Record stands as measured.)*
...
**Overhead audit (2026-09-18, `RUST_DFTB_PROF=evt`, GC N=86 batch=400, ~296 iters over 3 runs):** the harness is already lean — `finishes=97`, `reads=97` ≈ one queue drain + one status readback per 8-iter chunk boundary plus finalization, NOT per iteration. No in-loop allocs (only a per-chunk `work_list` rebuild). `scc.chunkend` host=575 ms is the *designed* wait-for-GPU, not wasted CPU. `scc.hscc` host=110 ms is enqueue backpressure (queue full → driver throttles the producer), not extra device work (dev 8.7 ms). **The device-time breakdown is what matters:**

| stage | dev ms | dev% | verdict |
|---|---:|---:|---|
| scc.jacobi | 430 | 69.6% | real work; already early-exits on off-norm/stall |
| scc.diis | 96.4 | 15.6% | **was the #2 kernel — a bug, fixed below** |
| 3× gemm (th/th2/sc) | ~73 | 11.8% | real GEMM work |
| everything else | ~12 | ~2% | noise |

So: no hidden per-iteration finishes, no redundant readbacks in the loop, no resident-data round-trips found. The remaining "harness" cost was inside a kernel.

**`diis_step_batched` QR moved to `__local` + cooperative threads (2026-09-18).** Root cause of the 15.6%: the pivoted f64 Gram–Schmidt ran **entirely on `lid==0`** (~6K serial f64 ops at 1/64 consumer f64 rate) while the other 255 lanes idled, *and* its working columns lived in a 928 KB global scratch `diis_work`. Fix: working columns + all solve arrays moved to `__local` (hist×n_atoms×8 B ≈ 2.3 KB), history loaded cooperatively, and the MGS dot/axpy restructured as **thread-per-column** (thread j owns column j — per-column serial order preserved → bit-identical results, pure re-scheduling); only pivot-select/normalize-broadcast and the ≤10×10 triangular solve stay on thread 0. Dead `diis_work` buffer/arg removed from both plans (arg slot 18 reused for `arg_local`, `work_ids` stays at 19). **Measured: `scc.diis` 96.4→46.6 ms (0.326→0.158 ms/call, ~2×); GC e2e 189.5→177.5 ms (~6%)**, all-converged, failed=0, `test_gpu_scc_plan_diis_parity_h2o` green. Remaining 0.158 ms/call is f64-rate-bound column work; further cuts need f32/Kahan QR or fewer barriers — diminishing returns, deferred.
...
- [ ] **f64-on-GPU audit — deferred sites must be re-verified for indirect damage (user-flagged 2026-09-18).** Policy: ANY f64 in a GPU kernel is a performance-policy violation unless deeply justified; where numerical accuracy is the justification, the justification must analyze whether a hybrid approach (f32 emulation / double-single, compensated or rearranged summation, f64 only for O(N) scalar decisions) can replace it. Audit table in `Measured_Facts_Jacobi_Sweeps.md` §4b. Deferred sites: `fermi_occ_batched` (f64 bisection — T10 Newton upgrade), fused Fermi tail (f64 exp+reductions), `diis_step_batched` QR (user decision: keep — barrier-bound), `energy_reduce_batched` (eval-only). Postponed ≠ cleared: re-verify each has no indirect cost (local-mem scratch, register pressure, instruction mix, occupancy) before final sign-off.
