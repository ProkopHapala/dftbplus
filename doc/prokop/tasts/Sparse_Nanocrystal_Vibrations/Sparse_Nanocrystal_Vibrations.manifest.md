...
- [ ] **L3. Measure P-locality directly, before K recovery.** `E_band = 2Tr(P·ZH)` — A=ZH is already built for P0, so `2Tr(P·A)` needs no globally-truncated K. Likewise q_A = 2Tr P_AA. Take the deg-386 reference, then purify P at deg 52/108/212 and compare E_P. If P is accurate at deg ~108 but K=PZ is not, the *hot SCC loop* can run at deg 64–128 and only short-range K/W blocks get built afterward for forces — a major win. The orthogonalization tail of S⁻¹ may make K intrinsically longer-ranged than P.
