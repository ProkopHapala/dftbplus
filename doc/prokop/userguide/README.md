...
- [sparse_vibrations.md](sparse_vibrations.md) — vibrational frequencies. §0 is the 2026-09-23 recipe: frozen-density vs FIRE-step Hessian, H/S-only blocks, 0.2 Å skin, 3ob carbon and pbc Si–H, DFTB+ parity on adamantane and Si₁₀H₁₆, C330 spectrum.
...
- [dftbcore_python.md](dftbcore_python.md) — the Python `DFTBcore` ctypes interface to the Fortran library: exporting H/S/DM/eigenvectors as NumPy arrays, the complex k-point getters for periodic systems, real-space bond-order analysis, and the Peierls/SSH worked examples (carbon chain, poly-p-phenylene).
- [bloch_slice.md](bloch_slice.md) — k-resolved real-space slices: project `C(k)` (not `C*`) with `project_bloch_points`, hue of one Bloch state versus a near-EF |\ψ|² sum. Drivers for the graphene sheet and the SPAMMM vacuum ribbons.
