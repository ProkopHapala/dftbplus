...
| GPU dense TC2 purification | OpenCL | `rust_dftb/src/qmqm/gpu_purify.rs` + `gpu_purify.cl` | **experimental** | eigensolve *alternative*: batched DM purification, 1 WG/system. Kernels `tc2_init_batched` (Palser init) + `tc2_step_batched` (fused: D² GEMM + ‖D²−D‖ + Tr + branch + `done[]` freeze), 1 launch/iter, zero syncs. Parity verified (n=86 b16: ΔE≈1e-6, ‖D−Dref‖≈3e-5). **Perf: 8.46 ms/solve at b400 cold — beats resident Jacobi (9.7 warm/17.1 cold)** — regtile-sq GEMM interior (22×11thr×8×4reg, As_t serves both operands, 6.7–7.8 TFLOPS ≈ 19–22 % peak isolated, tuned in `gpu_gemm.{rs,cl}`/`tests/gpu_gemm.rs`) + fused branch/write from acc registers. Warm-start (δK0 recipe) next. Design+status+code map: `tasts/HBond_Relaxed_Scan_GPU/Alternative_Dense_Multi_Eigensolve.md` |
