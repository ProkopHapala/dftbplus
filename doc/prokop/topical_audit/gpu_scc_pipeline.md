...
| OpenCL DM-purification | `rust_dftb/src/qmqm/gpu_purify.rs` + `gpu_purify.cl` | experimental | eigensolve alternative — batched dense TC2, produces D directly (no C/ε; occupation built in). 1 WG/system, fused step + device-side freeze, zero syncs. Parity-verified; not yet plan-integrated. Design: `tasts/HBond_Relaxed_Scan_GPU/Alternative_Dense_Multi_Eigensolve.md` |
