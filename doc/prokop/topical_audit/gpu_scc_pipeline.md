...
| OpenCL DM-purification | `rust_dftb/src/qmqm/gpu_purify.rs` + `gpu_purify.cl` | experimental | eigensolve alternative — batched dense TC2, produces D directly (no C/ε; occupation built in). Kernels `tc2_init_batched`/`tc2_step_batched`, 1 WG/system, fused step + device-side freeze, zero syncs. Parity-verified. **8.46 ms/solve n=86/b400 cold (beats resident Jacobi 9.7)** — regtile-sq GEMM interior (`gpu_gemm.cl::gemm_regtile` /sq mode) 6.7–7.8 TFLOPS ≈ 19–22 % peak. Not yet plan-integrated; warm-start next. Design+code map: `tasts/HBond_Relaxed_Scan_GPU/Alternative_Dense_Multi_Eigensolve.md` |
