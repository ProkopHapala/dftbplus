...
class GridShape:
    def _factorize(self, n):
        """Helper function to factorize a number into its prime factors."""
        i = 2
        factors = []
        while i * i <= n:
            while n % i == 0:
    ...
    def _find_fft_friendly_n(self, L, user_dx, tolerance=0.2, max_prime=7):
        """
        Find a number of grid points that:
        1. Results in a step size within tolerance of user_dx
        2. Factors completely into primes <= max_prime
        
        Args:
            L (float): Length of the cell in this direction
            user_dx (float): Desired grid step size
            tolerance (float): Maximum relative deviation from ideal step size
            max_prime (int): Maximum allowed prime factor
        
        Returns:
            tuple: (n, dx) where n is number of grid points and dx is actual step size
        """
        ideal_n = L / user_dx
        min_n = int(ideal_n * (1 - tolerance))
        max_n = int(ideal_n * (1 + tolerance))
        ...
        if best_n is None:
            raise ValueError(f"Could not find FFT-friendly grid size near {ideal_n} points")
            
        return best_n, L / best_n

    def compute_fft_friendly_shape(self, user_dx, tolerance=0.2, max_prime=7):
        """
        Compute grid shape with FFT-friendly dimensions.
        
        Args:
            user_dx (float): Desired grid step size
            tolerance (float): Maximum allowed deviation from ideal step size
            max_prime (int): Maximum allowed prime factor
        
        Returns:
            tuple: (dx, dy, dz, (nx, ny, nz)) - step sizes and grid points
        """
        # Get lattice dimensions from cell vectors
        Lx = np.linalg.norm(self.lvec[0])
        Ly = np.linalg.norm(self.lvec[1])
        Lz = np.linalg.norm(self.lvec[2])
        
        # Compute FFT-friendly grid dimensions
        nx, dx = self._find_fft_friendly_n(Lx, user_dx, tolerance, max_prime)
        ny, dy = self._find_fft_friendly_n(Ly, user_dx, tolerance, max_prime)
        nz, dz = self._find_fft_friendly_n(Lz, user_dx, tolerance, max_prime)
        
        print(f"DEBUG: FFT-friendly grid: nx={nx} ny={ny} nz={nz}")
        print(f"DEBUG: Effective steps: dx={dx:.6f} dy={dy:.6f} dz={dz:.6f}")
