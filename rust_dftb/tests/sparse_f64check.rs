...
fn intermediate_loss_diag() {
    ...
    let rk = std::env::var("F64CHECK_RK")
        .ok()
    ...
    let mut eng =
        SparseDftb::with_config(sk, &sk_dir, mol.species.clone(), mol.coords.clone(), cfg)
    ...
    let sm = (
        eng.s_bsr().unwrap().row_ptr.clone(),
        eng.s_bsr().unwrap().col_idx.clone(),
    );
    let sv: Vec<f64> = eng
        .s_bsr()
        .unwrap()
        ...
        .collect();
    let hm = (
        eng.h_bsr().unwrap().row_ptr.clone(),
        eng.h_bsr().unwrap().col_idx.clone(),
    );
    let hv: Vec<f64> = eng
        .h_bsr()
        .unwrap()
    ...
    eprintln!(
        "masked R_I: legacy={:.3e}  exact-products={:.3e}",
        ri(&q_leg, &kv),
        ri(&q_ext, &kv)
    );
    ...
    unsafe {
        lapack::dsyevd(
            b'V', b'L', n as i32, &mut a, n as i32, &mut w, &mut work, lw as i32, &mut iwork,
            li as i32, &mut info,
        );
    }
    assert_eq!(info, 0);
    ...
    for i in 0..n_atom {
        ...
        for bi in fm.0[i] as usize..fm.0[i + 1] as usize {
            ...
            for r in 0..norb[i] as usize {
                for c in 0..norb[j] as usize {
    ...
    let d_ext_ref: f64 = q_ext
        .iter()
        .zip(&q_ref)
        .map(|(a, b)| (a - b) * (a - b))
        .sum::<f64>()
        .sqrt();
    let d_leg_ref: f64 = q_leg
        .iter()
        .zip(&q_ref)
        .map(|(a, b)| (a - b) * (a - b))
        .sum::<f64>()
    ...
    eprintln!(
        "masked R_I: exact-K|M_K={:.3e}  sparse-K legacy={:.3e}  sparse-K exact-products={:.3e}",
        ri(&q_ref, &kv_ref_mk),
        ri(&q_leg, &kv),
        ri(&q_ext, &kv)
    );
    ...
    eprintln!(
        "E_band: sparse-K={e_dev:.6}  exact-K|M_K={:.6}",
        e_band(&kv_ref_mk)
    );
...
fn frozen_input_dense_ref() {
    ...
    let s_bsr = eng.s_bsr().unwrap().clone();
    let mut s_pad = vec![0.0f64; n_pad * n_pad];
    for i in 0..n_atom {
        for bi in s_bsr.row_ptr[i] as usize..s_bsr.row_ptr[i + 1] as usize {
            ...
            for r in 0..4 {
                for c in 0..4 {
                    s_pad[(i * 4 + r) * n_pad + j * 4 + c] =
                        s_bsr.values[bi * 16 + r * 4 + c] as f64;
                }
            }
        }
    }
    ...
    let h0_bsr = eng.h_bsr().unwrap().clone();
    ...
    for i in 0..n_atom {
        for bi in h0_bsr.row_ptr[i] as usize..h0_bsr.row_ptr[i + 1] as usize {
            ...
            for r in 0..4 {
                for c in 0..4 {
                    h0_pad[(i * 4 + r) * n_pad + j * 4 + c] =
                        h0_bsr.values[bi * 16 + r * 4 + c] as f64;
                }
            }
        }
    }
    ...
    eprintln!(
        "||2·K_sparse − D_exact||_F = {:.4}   ||D_exact||_F = {:.4}   ||K_exact=D/2||_F = {:.4}",
        diff.norm(),
        p_exact.norm(),
        p_exact.norm() / 2.0
    );
    eprintln!(
        "Tr(K_sparse·S) physical = {:.4}  (nocc={nocc})",
        trace_ps(&pd, &s_phys)
    );
    ...
    eprintln!(
        "host f64 ||ZS−I||_F/√N_pad = {:.3e}",
        (r_z / n_pad as f64).sqrt()
    );
    ...
    eprintln!(
        "E_band(exact K, masked to M_K) = {:.6}",
    ...
    let mut num = 0.0f64;
    let mut den = 0.0f64;
    for i in 0..ksk_e.len() {
        num += (ksk_e[i] - k_inj64[i]).powi(2);
        den += k_inj64[i].powi(2);
    }
    eprintln!(
        "exact-K f64 R_I (masked to M_K) = {:.3e}   (device floor was ~5e-4)",
        (num / den).sqrt()
    );
    ...
    let mut num = 0.0f64;
    let mut den = 0.0f64;
    for i in 0..ksk_s.len() {
        num += (ksk_s[i] - kv[i]).powi(2);
        den += kv[i].powi(2);
    }
    eprintln!(
        "sparse-K f64 R_I (masked to M_K) = {:.3e}",
        (num / den).sqrt()
    );

    let km_mat = Bsr4Matrix {
        n_atom,
        row_ptr: km.0.clone(),
        col_idx: km.1.clone(),
        values: k_inj,
    ...
    let (st, r_i, tr, it) = eng
        .purify_current()
        .unwrap_or_else(|e| panic!("purify injected: {e}"));
    ...
    eprintln!(
        "E_band after re-purify = {e_after:.6}  (exact-K band was {:.6})",
        2.0 * trace_ab_f64(&k_inj64, &km, &hv, &hm)
    );
}

fn off_at(norb: &[u8], i: usize) -> usize {
    norb[..i].iter().map(|&x| x as usize).sum()
}
