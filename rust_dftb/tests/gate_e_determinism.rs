...
fn test_gate_e_determinism_and_h_plateau() {
    ...
    for &tol in &tcs {
        ...
        let n_phys = ham.h0.nrows();
        let h0d: Vec<f64> = (0..n_phys * n_phys)
            .map(|i| ham.h0[(i / n_phys, i % n_phys)])
            .collect();
        let sd: Vec<f64> = (0..n_phys * n_phys)
            .map(|i| ham.s[(i / n_phys, i % n_phys)])
            .collect();
        ...
        let (z, _, _) = gpu
            .newton_schulz_inverse(&sb, &mask, &mask, 50, 1e-5, 5)
            .unwrap();
        let (emin, emax) = gpu.spectral_bounds(&hb, &z, &mask, 0.1).unwrap();
        let k0 = gpu
            .build_k0(&hb, &sb, &z, &mask, &mask, emin, emax)
            .unwrap();
        let (kf, _, _, _, _) = gpu
            .tc2_purify(&k0, &sb, n_occ as f32, &mask, &mask, &atom_n_orb, 80, tol)
            .unwrap();
        let kd = kf.to_dense();
    ...
    let tol_spread = e_by_tol
        .iter()
        .map(|e| (e - e_by_tol[0]).abs())
        .fold(0.0f64, f64::max);
    ...
    let rel_bias = if max_force > 1e-12 {
        max_bias / max_force
    } else {
        0.0
    };
