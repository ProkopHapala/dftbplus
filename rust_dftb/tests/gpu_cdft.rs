...
fn test_cdft_h2o_fragment_charge() {
    ...
    eprintln!(
        "[cdft] outer={} q_err_max={:.3e} Q_O={:.6} λ={:.4} conv={:?}",
        rep.outer_iters, rep.q_err_max, rep.qfrag[0], rep.lam[0], rep.converged
    );
    assert!(
        rep.converged[0],
        "CDFT did not converge: q_err={:.3e}",
        rep.q_err_max
    );
    assert!(
        (rep.qfrag[0] - target).abs() < 1e-3,
        "Q_O={} != {target}",
        rep.qfrag[0]
    );
    ...
    eprintln!(
        "[cdft] constrained E={e_c:.8} (raw eval incl. λ·Q = {:.8})",
        eng.eval(false).unwrap().energy[0]
    );
    assert!(
        e_c > e0 - 1e-5,
        "constrained E={e_c} < unconstrained E0={e0} — constraint not a penalty"
    );
    ...
    eprintln!(
        "[cdft] cleared E={e_back:.8} vs E0={e0:.8} |dE|={:.3e}",
        (e_back - e0).abs()
    );
    assert!(
        (e_back - e0).abs() < 1e-5,
        "clear_cdft did not restore baseline: |dE|={:.3e}",
        (e_back - e0).abs()
    );
}
...
fn test_cdft_h2o_batched_ladder() {
    ...
    let rep = eng.cdft_scc(30, 60, 1e-6, 1e-4).unwrap();
    eprintln!(
        "[cdft] ladder: outer={} q_err_max={:.3e}",
        rep.outer_iters, rep.q_err_max
    );
    for b in 0..batch {
        eprintln!(
            "[cdft]   b={b} target={:.2} Q={:.6} λ={:.4} conv={}",
            targets[b], rep.qfrag[b], rep.lam[b], rep.converged[b]
        );
        assert!(
            rep.converged[b],
            "replica {b} unconverged: Q={} target={}",
            rep.qfrag[b], targets[b]
        );
