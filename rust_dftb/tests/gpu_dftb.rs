...
fn test_gpu_dftb_constraint_and_frozen() {
    ...
    // --- invalid inputs must fail loud (before any state mutation) ---
    {
        ...
        assert!(eng.set_constraint(1, 7, &[1.5]).is_err(), "out-of-range must fail");
        assert!(eng.set_constraint(1, 2, &[]).is_err(), "targets.len != batch must fail");
        assert!(eng.set_constraint(1, 2, &[0.0]).is_err(), "zero target must fail");
        assert!(eng.set_constraint(1, 2, &[-1.0]).is_err(), "negative target must fail");
        assert!(eng.set_constraint(1, 2, &[f64::NAN]).is_err(), "NaN target must fail");
        eng.set_frozen_atoms(&[1, 2]).unwrap();
        assert!(eng.set_constraint(1, 2, &[1.5]).is_err(), "both-frozen must fail at set_constraint");
    }
    // freezing both endpoints AFTER the constraint exists must also fail
    {
        let mut eng = GpuDftb::new(sk.clone(), &dir, sp.clone(), xyz.clone(), 1).unwrap();
        eng.set_constraint(1, 2, &[1.6]).unwrap();
        assert!(eng.set_frozen_atoms(&[1, 2]).is_err(), "freezing both endpoints post-hoc must fail");
    }
    ...
    assert!((d1 - 1.6).abs() < 1e-5, "initial projection failed: dist={d1}");
    ...
    for step in 0..5 {
        ...
        assert!((d - 1.6).abs() < 1e-4, "constraint violated at step {step}: dist={d}");
        ...
        for k in 0..2 {
            let dx: f64 = (0..3).map(|c| (eng.coords()[k][c] - c_frozen[k][c]).powi(2)).sum::<f64>().sqrt();
...
fn test_gpu_dftb_masked_retry_isolation() {
    ...
    eprintln!("[R2] run-1: E={:?} iters={} statuses={:?}", e1.energy, s1.n_iters, s1.statuses);
    ...
    eprintln!("[R2] masked retry rep1: E={:?} iters={} statuses={:?}", e2.energy, s2.n_iters, s2.statuses);
    assert_eq!(e2.energy[0], e1.energy[0], "replica 0 energy changed under masked retry — its state was touched");
    assert!((e2.energy[1] - e1.energy[1]).abs() < 1e-5, "replica 1 re-solve landed elsewhere: dE={:.3e}", e2.energy[1] - e1.energy[1]);

    // Exhausted SCC (max_iter=1, impossible tol) → all Failed, honestly.
    let s3 = eng.scc(1, 1e-12).unwrap();
    eprintln!("[R2] exhausted: statuses={:?} stalled={} rms={:.3e}", s3.statuses, s3.stalled, s3.rms);
    assert!(s3.stalled, "exhausted SCC must report stalled");
    assert!(s3.statuses.iter().all(|s| *s == SccStatus::Failed), "exhausted SCC must report Failed: {:?}", s3.statuses);
    ...
    for (b, (a, b0)) in eng.coords().iter().zip(c0.iter()).enumerate() {
        for c in 0..3 {
            assert!((a[c] - b0[c]).abs() < 1e-6, "replica {b} atom moved on uncertified SCC state");
        }
    }
...
fn test_gpu_dftb_jacobi_capacity_guard() {
    ...
    let cases: [(usize, usize, usize, bool); 3] = [(64, 0, 0, true), (62, 1, 1, true), (65, 0, 0, false)];
    for (nc, nn, nh, ok) in cases {
        ...
        match GpuDftb::new(sk, &dir, sp, coords, 1) {
            Ok(_) if !ok => panic!("n={n_orb} (jpair={}) accepted — R5 capacity guard missing", (n_orb + 1) / 2),
            ...
            Err(e) => {
                ...
                assert!(e.to_string().contains("capacity"), "rejection should name the capacity limit: {e}");
            }
        }
    }
}
...
fn check_edm_fresh(kt: f32) {
    ...
    eng.set_smearing(kt);
    let s = eng.scc(100, 1e-6).unwrap_or_else(|e| panic!("batch scc: {e}"));
    eprintln!("[R1 kT={kt}] batch scc iters={} rms={:.3e} statuses={:?}", s.n_iters, s.rms, s.statuses);
    ...
    for (b, &d) in stretch.iter().enumerate() {
        ...
        e1.set_smearing(kt);
        let s1 = e1.scc(100, 1e-6).unwrap_or_else(|e| panic!("solo scc {b}: {e}"));
        let ev1 = e1.eval(true).unwrap_or_else(|e| panic!("solo eval {b}: {e}"));
        ...
        if mb > max_df { max_df = mb; worst = b; }
    }
