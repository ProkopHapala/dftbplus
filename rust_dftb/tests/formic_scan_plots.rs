...
fn per_atom_u(sk: &rust_dftb::SkData, species: &[String]) -> Vec<f64> {
    species
        .iter()
        .map(|sp| sk.onsite(sp).unwrap().u_hubbard)
        .collect()
}
...
fn run_gpu_scan(
    ...
    eprintln!(
        "  [GPU] SCC done: n_iters={}, {n_conv}/{n_points} converged, max|E|={:.6}",
        gpu.n_iters,
        gpu.energies.iter().fold(0.0f32, |m, &e| m.max(e.abs()))
    );
...
fn save_1d_data(
    ...
    for i in 0..t_values.len() {
        write!(
            f,
            "{:.4}  {:.8}  {:.8}",
            t_values[i], cpu_energies[i], gpu_energies[i]
        )
        .unwrap();
        for &atom in &[h1, d1, a1, h2, d2, a2] {
            write!(
                f,
                "  {:.6}  {:.6}",
                cpu_charges[i * n_atoms + atom],
                gpu_charges[i * n_atoms + atom]
            )
            .unwrap();
        }
        writeln!(f).unwrap();
...
fn save_2d_data(
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            let e_str = if energies[idx].is_nan() {
                "NaN".to_string()
            } else {
                format!("{:.8}", energies[idx])
            };
            let q_str = if energies[idx].is_nan() {
                "NaN".to_string()
            } else {
                format!("{:.6}", charges[idx * n_atoms + atom_idx])
            };
            writeln!(
                f,
                "{i}  {j}  {:.4}  {:.4}  {e_str}  {q_str}  {conv}",
...
fn test_formic_dimer_scan_plots() {
    ...
    let n_1d = 41usize;
    let t_1d: Vec<f64> = (0..n_1d)
        .map(|i| 2.0 * i as f64 / (n_1d - 1) as f64)
        .collect();
    let geoms_1d: Vec<Vec<[f64; 3]>> = t_1d
        .iter()
    ...
    save_1d_data(
        &path_1d, &t_1d, &cpu_e_1d, &gpu_e_1d, &cpu_q_1d, &gpu_q_1d, n_atoms, h1, h2, d1, a1, d2,
        a2,
    );
    ...
    let n_2d = 21usize;
    let t_2d: Vec<f64> = (0..n_2d)
        .map(|i| 2.0 * i as f64 / (n_2d - 1) as f64)
        .collect();
    ...
    for i in 0..n_2d {
        ...
        for j in 0..n_2d {
            ...
            if strip_rms[j] < 5e-6 {
                ...
                eprintln!(
                    "  [GPU] 2D point ({t1:.1},{:.1}) UNCONVERGED: RMS={:.2e}",
                    t_2d[j], strip_rms[j]
                );
            }
        }
