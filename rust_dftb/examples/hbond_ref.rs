...
use rust_dftb::methods::dftb::forces::{
    compute_scc_forces, parse_repulsive_spline, RepulsiveSpline,
};
...
fn make_geom_1d(
    ...
    coords[h1] = interp_h_pos(
        &base_coords[h1],
        &base_coords[donor1],
        &base_coords[acceptor1],
        t,
    );
    coords[h2] = interp_h_pos(
        &base_coords[h2],
        &base_coords[donor2],
        &base_coords[acceptor2],
        t,
    );
    coords
}
...
fn save_point_data(
    ...
    writeln!(
        f,
        "# t1 t2 electronic_scc_energy repulsive_energy total_energy n_iter"
    )?;
    writeln!(
        f,
        "{:.16e} {:.16e} {:.16e} {:.16e} {:.16e} {}",
...
impl FireOptimizer {
    ...
    fn step(&mut self, coords: &[[f64; 3]], forces: &[[f64; 3]]) -> (Vec<[f64; 3]>, f64) {
        ...
        let scale = if f2 > 1e-24 {
            self.alpha * v2.sqrt() / f2.sqrt()
        } else {
            0.0
        };
        for i in 0..n {
            for c in 0..3 {
                self.velocities[i][c] =
                    (1.0 - self.alpha) * self.velocities[i][c] + scale * forces[i][c];
            }
        }
...
fn optimize_geometry_persistent(
    ...
    use rust_dftb::methods::dftb::dftb_cpu::{CpuSccResult, DftbCpu};
    ...
    let sk = load_sk_for_species(sk_dir, species).map_err(|e| format!("Failed to load SK: {e}"))?;
    ...
    let mut solver =
        DftbCpu::new(sk, species.to_vec()).map_err(|e| format!("DftbCpu::new failed: {e}"))?;
    ...
    eprintln!(
        "  [opt] FIRE: dt0={}, dt_max={}, vmax={}, max_disp=0.1Å, max_iter={}, f_tol={:.2e}",
        1.0, 5.0, 2.0, max_opt_iter, f_tol
    );
    eprintln!(
        "  [opt] SCC: max_iter={}, tol={:.2e} (warm-started, persistent state)",
        scc_max_iter, scc_tol
    );
    eprintln!(
        "  [opt] frozen atoms: {:?}",
    ...
    solver
        ...
        .map_err(|e| format!("update_geometry failed: {e}"))?;
    ...
    let mut traj_file = if let Some(p) = traj_path {
        ...
        Some(
            File::create(p)
                .unwrap_or_else(|e| panic!("Cannot create trajectory {}: {e}", p.display())),
        )
    } else {
        None
    };
    let mut hist_file = if let Some(p) = hist_path {
        ...
        let mut f = File::create(p)
            .unwrap_or_else(|e| panic!("Cannot create history {}: {e}", p.display()));
    ...
    while iter < max_opt_iter && max_f > f_tol {
        ...
        solver
            .solve_scc(scc_max_iter, scc_tol)
        ...
        if let Some(tf) = &mut traj_file {
            ...
            writeln!(
                tf,
                "iter={} E={:.10e} max_F={:.6e} rms_F={:.6e}",
                iter, last_energy, max_f, rms_f
            )
            .unwrap();
        ...
        solver
            .update_geometry(&current)
            .map_err(|e| format!("update_geometry failed at iter {iter}: {e}"))?;
    ...
    eprintln!(
        "  [opt] DONE: {} iters, {:.2}s, final E={:.10e}",
        iter,
        elapsed.as_secs_f64(),
        last_energy
    );
...
fn optimize_geometry(
    ...
    if !restraints.is_empty() {
        ...
        for (i, target, k) in restraints {
            eprintln!(
                "  [opt]   atom {} → ({:.3},{:.3},{:.3}) k={:.1}",
                i, target[0], target[1], target[2], k
            );
        }
    }
    ...
    while iter < max_opt_iter && max_f > f_tol {
        let t0 = std::time::Instant::now();
        let scc = builder
            .build_scc(species, &current, scc_max_iter, scc_tol)
            .map_err(|e| format!("SCC failed at opt iter {iter}: {e}"))?;
        let t_scc = t0.elapsed();
        last_energy = scc.energy;
    ...
    if max_f <= f_tol {
        eprintln!(
            "  [opt] CONVERGED in {} iterations ({:.1}s), max|F|={:.6e} <= tol={:.2e}",
            iter,
            elapsed.as_secs_f64(),
            max_f,
            f_tol
        ...
        eprintln!(
            "  [opt] REACHED max_iter={} ({:.1}s), max|F|={:.6e}",
            max_opt_iter,
            elapsed.as_secs_f64(),
            max_f
        );
...
fn main() {
    ...
    // Parse geometry
    let mol = parse_xyz(&args.xyz).unwrap_or_else(|e| panic!("Failed to parse {}: {e}", args.xyz));
    ...
    println!(
        "H1: atom {} moves donor({}) → acceptor({})",
        args.h1, args.donor1, args.acceptor1
    );
    println!(
        "H2: atom {} moves donor({}) → acceptor({})",
        args.h2, args.donor2, args.acceptor2
    );

    // Load SK data
    let sk =
        load_sk_for_species(&sk_dir, &species).unwrap_or_else(|e| panic!("Failed to load SK: {e}"));
    ...
    if args.mode == "optimize" {
        ...
        save_xyz(
            ...
            &opt_coords,
            &format!(
                "optimized: E={:.10e} Hartree, {} FIRE iters",
                opt_energy, n_iter
            ),
        );
    ...
    if args.mode == "switch" {
        ...
        save_xyz(
            ...
            &species,
            &switched,
            "switched H atoms (before optimization)",
        );
        ...
        save_xyz(
            ...
            &prod_coords,
            &format!(
                "product optimized: E={:.10e} Hartree, {} FIRE iters",
                prod_energy, n_iter
            ),
        );
        ...
        let scc_r = builder
            .build_scc(&species, &r_coords, args.max_iter, args.tol)
        ...
        println!("\n=== Results ===");
        println!(
            "Reactant:  E_elec={:.10e}  E_rep={:.10e}  E_total={:.10e}",
            scc_r.energy, e_rep_r, e_total_r
        );
        println!(
            "Product:   E_elec={:.10e}  E_rep={:.10e}  E_total={:.10e}",
            prod_energy, e_rep_p, e_total_p
        );
        println!(
            "ΔE = {:.6e} Hartree = {:.2} kcal/mol = {:.4} eV",
            e_total_p - e_total_r,
    ...
    for idx in 0..n_total {
        ...
        let scc = match builder.build_scc(&species, &coords, args.max_iter, args.tol) {
            ...
            Err(e) => {
                eprintln!(
                    "  WARNING: SCC failed at pt {} (t1={:.3} t2={:.3}): {} — skipping",
                    idx, t1, t2, e
                );
        ...
        // Save per-point data
        save_point_data(
            &data_dir, idx, t1, t2, &species, &coords, &scc, e_rep, e_total,
        )
        ...
        if is_2d {
            writeln!(
                csv,
                "{},{:.6},{:.6},{:.16e},{:.16e},{:.16e},{}",
                idx, t1, t2, scc.energy, e_rep, e_total, scc.n_iter
            )
            .unwrap();
        } else {
            writeln!(
                csv,
                "{},{:.6},{:.16e},{:.16e},{:.16e},{}",
                idx, t1, scc.energy, e_rep, e_total, scc.n_iter
            )
            .unwrap();
        }
        ...
        if idx % 5 == 0 || idx == n_total - 1 {
            println!(
                "  pt {}/{}: t1={:.3} t2={:.3} E_total={:.10e} n_iter={}",
                idx + 1,
                n_total,
                t1,
                t2,
    ...
    println!(
        ...
        n_total,
        elapsed.as_secs_f64(),
        elapsed.as_secs_f64() / n_total as f64
    );
