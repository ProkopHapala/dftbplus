...
/// Created once per `build_non_scc` call; eliminates per-pair allocations.
pub struct HWorkspace {
    /// Preallocated flat buffer for H diatomic block
    pub block_h: Vec<f64>,
    /// Preallocated flat buffer for S diatomic block
    pub block_s: Vec<f64>,
    /// Max block size (max_n_orb * max_n_orb) in this system
    pub max_block: usize,
}
...
impl HWorkspace {
    ...
    /// Get slices of the exact size needed for a (ni, nj) block.
...
impl HamiltonianBuilder {
    ...
    fn fill_pairs(
        ...
        // Max orbital count among all species → max block size
